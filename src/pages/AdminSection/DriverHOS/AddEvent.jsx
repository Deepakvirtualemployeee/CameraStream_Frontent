...
export const AddEvent = () => {
  ...
  const [eventDateForDb, setEventDateForDb] = useState(null); // to store UTC date for backend
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      addEditEvent(companyId, driverId, null, payload, selectedDate, navigate)
    );
  };
  ...
  useEffect(() => {
    if (timeZoneId) {
      setEventDate(moment.tz(timeZoneId).toDate());
    }
  }, [timeZoneId]);
                    ...
                    <div className="w-100">
                      ...
                      >
                        <RsuiteDatePicker
                          ...
                          value={
                            eventDate
                              ? moment(eventDate).tz(timeZoneId).toDate()
                              : null
                          }
                          onChange={(date) => {
                            if (!date) return;
